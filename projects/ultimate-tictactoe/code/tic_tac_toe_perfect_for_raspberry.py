...
def handle_mouse_click(event):
    ...
    if GLOBALS.mode_tracking == 0:
        params = get_inputs(mouse)      
    elif GLOBALS.mode_tracking == 1:
        params = (GLOBALS.lb_for_hand, GLOBALS.local_row, GLOBALS.local_col)
                ...
                if GLOBALS.turn == GLOBALS.bot :  # bot turn
                    if GLOBALS.bot_difficulty == 'Easy' :
                        ...
                        if local_boards == [[[[0 for _ in range(3)] for _ in range(3)] for _ in range(3)] for _ in range(3)]:
                            ...
                            _,move1=alV6.minimax(local_boards,GLOBALS.prev_lb.index,GLOBALS.prev_local_row,GLOBALS.prev_local_col,str(GLOBALS.bot),3,list_tic)  
                        ...
                        if local_boards == [[[[0 for _ in range(3)] for _ in range(3)] for _ in range(3)] for _ in range(3)]:
                            ...
                            if not valid_moves:
                                ...
                                _,move1=alV6.minimax(local_boards,GLOBALS.prev_lb.index,GLOBALS.prev_local_row,GLOBALS.prev_local_col,str(GLOBALS.bot),5,list_tic)
                ...
                if GLOBALS.turn == GLOBALS.player: # Local player turn
                    ...
                    if local_boards == [[[[0 for _ in range(3)] for _ in range(3)] for _ in range(3)] for _ in range(3)]:
                        ...
                        if not valid_moves:
                            ...
                            _,move1=alV6.minimax(local_boards,GLOBALS.prev_lb.index,GLOBALS.prev_local_row,GLOBALS.prev_local_col,str(GLOBALS.bot),depth,list_tic)                       
                        lb, row, col = GLOBALS.global_board.local_board_list[move1[0]],move1[1],move1[2]
                    play = str(lb.index)+str(row*3+col)
                    com.send_play(GLOBALS.socket, play, GLOBALS.state_of_play, GLOBALS.turn)
                    make_move(lb, row, col) 
            ...
            if GLOBALS.gamemode == ONLINE: # Mode online
                ...
                if GLOBALS.turn == GLOBALS.player: # Local player turn
                    ...
                    if local_boards == [[[[0 for _ in range(3)] for _ in range(3)] for _ in range(3)] for _ in range(3)]:
                        ...
                        box_won = alV6.update_box_won(local_boards)
                        valid_moves = [(i, j, x, y) for i in range(3) for j in range(3) for x in range(3) for y in range(3) if al.is_valid_move(list_tic, local_boards, i, j, x, y, GLOBALS.prev_local_row, GLOBALS.prev_local_col)]
                        if not valid_moves:
                            _,move1=alV6.minimax(local_boards,GLOBALS.prev_lb.index,GLOBALS.prev_local_row,GLOBALS.prev_local_col,str(GLOBALS.bot),3,box_won)
                        else:
                            _,move1=alV6.minimax(local_boards,GLOBALS.prev_lb.index,GLOBALS.prev_local_row,GLOBALS.prev_local_col,str(GLOBALS.bot),depth,box_won)                       
                        lb, row, col = GLOBALS.global_board.local_board_list[move1[0]],move1[1],move1[2]
                    play = str(lb.index)+str(row*3+col)
                    com.send_play(GLOBALS.socket, play, GLOBALS.state_of_play, GLOBALS.turn)
                    make_move(lb, row, col)  
...
def main() -> None:
    if not GLOBALS.cam_open:

        x = threading.Thread(target=hand_tracking, daemon=True)
        x.start()
        GLOBALS.cam_open = True
